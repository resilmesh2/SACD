...
export class FlowEditorComponent implements OnInit {
  ...
  public deleteSelected(): void {
    ...
    // Deleting root node deletes mission from the map
    Object.keys(this.missionsMap()).forEach((missionId) => {
      if (this.selected().includes(missionId)) {
  ...
  addNewMissionRoot() {
    ...
    this.missionsMap.update((map) => {
      return {
        ...map,
        [newRootId]: { name: '', description: '', criticality: 1 },
      };
    });
  }
  ...
  public addComponentGroup(): void {
    ...
    this.selectNodes([
      ...
      `f-node-${componentAId}`,
      `f-node-${componentBId}`,
      `f-node-${componentAAddId}`,
      `f-node-${componentBAddId}`,
    ]);
  }
  ...
  findEmptyXPositionAtLayer(layer: AggregationLayer): number {
    ...
    const usedXPositions = nodesAtLayer
      .map((node) => {
  ...
  public addHostNode(hostname?: string, ip?: string, x?: number): number {
    ...
    this.selectNodes([`f-node-${hostId}`]);
    return hostId;
  }
