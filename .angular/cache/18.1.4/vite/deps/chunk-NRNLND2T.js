...
function nest_default() {
  ...
  function entries(map2, depth) {
    ...
    if (rollup != null && depth >= keys.length) array = map2.entries();
    else array = [], map2.each(function(v, k) {
      array.push({
        key: k,
...
function link_default(links) {
  var id = index, strength = defaultStrength, strengths, distance = constant_default(30), distances, nodes, count, bias, iterations = 1;
  if (links == null) links = [];
  ...
  function initialize() {
    if (!nodes) return;
    var i, n = nodes.length, m = links.length, nodeById = map_default(nodes, id), link;
...
function simulation_default(nodes) {
  var simulation, alpha = 1, alphaMin = 1e-3, alphaDecay = 1 - Math.pow(alphaMin, 1 / 300), alphaTarget = 0, velocityDecay = 0.6, forces = map_default(), stepper = timer(step), event = dispatch_default("tick", "end");
  if (nodes == null) nodes = [];
  function step() {
    tick();
...
function manyBody_default() {
  var nodes, node, alpha, strength = constant_default(-30), strengths, distanceMin2 = 1, distanceMax2 = Infinity, theta2 = 0.81;
  function force(_) {
...
//# sourceMappingURL=chunk-NRNLND2T.js.map
